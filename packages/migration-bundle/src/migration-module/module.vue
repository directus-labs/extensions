<template>
	<private-view title="Migrate Directus">
		...
		<template #navigation>
			<support-navigation :collection="`home`"/>
		</template>
			...
			<div class="migration-main">
				...
				<div class="migration-input-container">
					<div class="migration-input">
						<v-input label="Destination URL" placeholder="https://" :disabled="isValidating || lockInterface" v-model="baseURL"/>
						<v-input label="Admin Token" placeholder="**********" :disabled="isValidating || lockInterface" v-model="token"/>
						<v-button :disabled="lockInterface" :loading="isValidating" @click="checkHost({ baseURL, token })">Check</v-button>
					</div>

					<div v-if="validationMessage" class="migration-validation">
						<v-notice :icon="validationMessage.icon" :type="validationMessage.status">{{ validationMessage.message }}</v-notice>
						<v-checkbox
					...
					<div class="migration-start" v-if="forceSchema || (validationMessage && validationMessage.status == 'success')">
						<div class="migration-options">
							<v-select
								...
								:disabled="lockInterface"
								:multiple="true"
								:multiplePreviewThreshold="1"
								itemText="label"
								itemValue="value"
								itemIcon="icon"
								allItemsTranslation="Migration Options"
								itemCountTranslation="Migration Options"
						...
						<v-checkbox
							...
							:disabled="lockInterface"
						>Dry Run</v-checkbox>
						<v-button :disabled="lockInterface" @click="extractSchema({ baseURL, token, dryRun })">Start</v-button>
					</div>
				</div>

				<div class="migration-transcript" v-html="md(dataChunk)"></div>
...
<style>
	...
	h3.done .icon, h3.skipped .icon, h3.error .icon {
		position: relative;
	...
	h3.done .icon i, h3.error .icon i, h3.skipped .icon i {
		display: block;
	...
	@keyframes progress-circular-stroke {
		0% { stroke-dasharray: 0, 78.5px; }
		50% { stroke-dasharray: 78.5px, 78.5px; }
		100% { stroke-dasharray: 0, 78.5px; }
	}
