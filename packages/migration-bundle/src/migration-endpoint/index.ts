...
import { initLimiter, enhancedFetch, Schema } from "./api";
...
import { validate_system, validate_data, validate_migration } from "../utils/validate";
import { migrateSchema, checkSchema } from "./composables/migrate-schema";
		...
		router.post('/*', async (req, res) => {
			...
			if(isCheck){
				...
				if(response && "errors" in response){
					let message = Array.isArray(response.errors) && response.errors.length > 0 ? response.errors[0]?.message : "Unknown";
					res.json({ status: "danger", icon: "error", message: message });
				} else {
				...
				try {
					...
					if(systemFetch.system_errors){
						let message = Array.isArray(systemFetch.system_errors.errors) && systemFetch.system_errors.errors.length > 0 ? systemFetch.system_errors.errors[0]?.message : "Unknown";
					...
					isValid = isValid ? await validate_data({
						...dataFetch,
						scope,
					}) : isValid;
