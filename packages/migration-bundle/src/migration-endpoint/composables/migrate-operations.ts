...
const migrateOperations = async ({ res, client, operations, dry_run = false }: { res: any, client: RestClient<Schema>, operations: OperationRaw[] | null, dry_run: boolean }): Promise<{ response: string, name: string } | DirectusError> => {
