...
import type { DirectusError, CommentRaw } from '../../types/extension';
import { createComments, readComments } from '@directus/sdk';
import { Schema } from '../api';

const migrateComments = async ({ res, client, comments, dry_run = false }: { res: any, client: RestClient<Schema>, comments: CommentRaw[] | null, dry_run: boolean }): Promise<{ response: string, name: string } | DirectusError> => {
  ...
  try {
    ...
    if(!existingComments) return { name: "Directus Error", status: 404, errors: [{ message: "Issue Fetching Comments"}]};
    res.write("done\r\n\r\n");
