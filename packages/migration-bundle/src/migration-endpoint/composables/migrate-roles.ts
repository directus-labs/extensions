...
import { readRoles, createRole, updateRole } from '@directus/sdk';
import { Schema } from '../api';

const migrateRoles = async ({ res, client, roles, dry_run }: { res: any, client: RestClient<Schema>, roles: RoleRaw[] | null, dry_run: boolean }): Promise<{ response: string, name: string } | DirectusError> => {
  ...
  if(!existingRoles) return { name: "Directus Error", status: 404, errors: [{ message: "No Roles found"}]};
  res.write("done\r\n\r\n");
