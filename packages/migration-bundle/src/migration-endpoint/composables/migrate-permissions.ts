...
import { createPermission, readPermissions } from '@directus/sdk';

async function migratePermissions({ res, client, permissions, dry_run = false }: { res: any; client: RestClient<Schema>; permissions: Permission[] | null; dry_run: boolean }): Promise<{ response: string; name: string } | DirectusError> {
	if (!permissions) {
