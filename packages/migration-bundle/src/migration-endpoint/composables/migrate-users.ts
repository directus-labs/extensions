...
const migrateUsers = async ({ res, client, users, roles, dry_run = false }: { res: any, client: RestClient<Schema>, users: UserRaw[] | null, roles: RoleRaw[] | null, dry_run: boolean }): Promise<{ response: string, name: string } | DirectusError> => {
  ...
  try {
    ...
    if(!existingUsers) return { name: "Directus Error", status: 404, errors: [{ message: "No Users found in destination"}]};
    res.write("done\r\n\r\n");
