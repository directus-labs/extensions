...
async function updateRequiredFields({ res, client, service, collections, dry_run = false, task }: { res: any; client: RestClient<Schema>; service: any; collections: Collection[] | null; dry_run: boolean; task: 'add' | 'remove' }): Promise<{ response: string; name: string } | DirectusError> {
...
function removeRequiredorIsNullable(field: Field) {
