...
import { readPanels, createPanel } from '@directus/sdk';
import { Schema } from '../api';

const migratePanels = async ({ res, client, panels, dry_run = false }: { res: any, client: RestClient<Schema>, panels: PanelRaw[] | null, dry_run: boolean }): Promise<{ response: string, name: string } | DirectusError> => {
  ...
  try {
    ...
    if(!existingPanels) return { name: "Directus Error", status: 404, errors: [{ message: "Issue Fetching Panels"}]};
    res.write("done\r\n\r\n");
