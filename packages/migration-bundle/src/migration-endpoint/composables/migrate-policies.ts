...
async function migratePolicies({ res, client, policies, dry_run = false }: { res: any; client: RestClient<Schema>; policies: PolicyRaw[] | null; dry_run: boolean }): Promise<{ response: string; name: string } | DirectusError> {
	if (!policies) {
