...
import type { DirectusError, ModifiedFlowRaw, OperationRaw } from '../../types/extension';
import type { Schema } from '../api';
import { createFlow, readFlows } from '@directus/sdk';
import migrateOperations from './migrate-operations';

async function migrateFlows({ res, client, flows, operations, dry_run = false }: { res: any; client: RestClient<Schema>; flows: ModifiedFlowRaw[] | null; operations: OperationRaw[] | null; dry_run: boolean }): Promise<{ response: string; name: string } | DirectusError> {
	if (!flows || !operations) {
	...
	try {
		...
		if (!existingFlows)
			return { name: 'Directus Error', status: 404, errors: [{ message: 'Issue Fetching Flows' }] };
		res.write('done\r\n\r\n');
