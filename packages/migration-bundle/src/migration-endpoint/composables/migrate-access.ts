...
import type { Access, DirectusError, RoleRaw } from '../../types/extension';
import type { Schema } from '../api';
import getRoleIds from '../../utils/get-role-ids';

async function migrateAccess({ res, client, access, roles, dry_run = false }: { res: any; client: RestClient<Schema>; access: Access[] | null; roles: RoleRaw[] | null; dry_run: boolean }): Promise<{ response: string; name: string } | DirectusError> {
	if (!access || !roles) {
	...
	try {
		...
		if (!existingAccesses)
			return { name: 'Directus Error', status: 404, errors: [{ message: 'Issue Fetching Access' }] };
		res.write('done\r\n\r\n');
