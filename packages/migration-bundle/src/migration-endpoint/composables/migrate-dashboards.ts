...
import type { DirectusError, DashboardRaw } from '../../types/extension';
import { readDashboards, createDashboard } from '@directus/sdk';
import { Schema } from '../api';

const migrateDashboards = async ({ res, client, dashboards, dry_run = false }: { res: any, client: RestClient<Schema>, dashboards: DashboardRaw[] | null, dry_run: boolean }): Promise<{ response: string, name: string } | DirectusError> => {
  ...
  try {
    ...
    if(!existingDashboards) return { name: "Directus Error", status: 404, errors: [{ message: "Issue Fetching Dashboards"}]};
    res.write("done\r\n\r\n");
