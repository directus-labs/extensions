  ...
  <private-view title="Search">
    ...
    <main class="container">
      ...
      <header v-if="settingsValid">
        <form class="search-form" @submit.prevent="search(query)">
          ...
          <v-button v-if="settings.searchMode === 'form_submit'" type="submit"
            >Search
          </v-button>
        </form>
        <div class="search-info">
          <p v-show="query">
            {{ info.hits }} results returned in {{ info.time }}ms for "{{
              info.query
      ...
      <section v-if="!settingsValid">
        ...
        >
          ...
          <template v-if="isAdmin">
            <p>Please configure the command palette settings.</p>
            <v-button
              :to="{ name: 'global-search-settings' }"
          ...
          <p v-else>
            Please contact an admin to properly configure the command palette.
          </p>
        </v-info>
      </section>
      ...
      <section v-if="settingsValid && query" class="search-results">
        ...
        <template v-else>
          <v-info title="No Results Found" icon="block"
    ...
    <template #sidebar>
      ...
      <sidebar-detail icon="history" title="Recent Searches" start-open>
        <div
          v-md="
            `Recent searches are only stored locally on your browser and not in the database.`
          "
          class="page-description"
        />
        <recent-searches
