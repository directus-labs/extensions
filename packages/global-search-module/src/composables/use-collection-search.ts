...
type UseCollectionSearchOptions = {
...
export function useCollectionSearch(
  ...
  watch([collection, search], ([_, newSearch], [__, oldSearch]) => {
    if (!collectionSearchInfo.value) return;
    if (performedQueries.has(newSearch as string)) return;
  ...
  async function fetch(limit: number, searchFilter: Filter | null) {
    ...
    const query = {
      ...
      filter: mergeFilters(filter, searchFilter),
      fields: [
