...
const parts = computed<HighlightPart[]>(() => {
	...
	mergedMatches.forEach(([start, end], index) => {
		const precedingText = props.text.slice(0, start).split(' ').slice(-3).join(' ');
		const followingText = props.text.slice(end).split(' ').slice(0, 3).join(' ');
		const highlightedText = props.text.slice(start, end);
