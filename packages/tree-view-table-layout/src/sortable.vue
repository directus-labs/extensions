...
<script setup lang="ts">
    import { ref, computed, provide, type ModelRef, type Ref } from "vue";
    ...
    function useSortable({
        ...
        function getDraggedChildren(): Item[] {
            if (!draggedItemId.value || !nestable.value) return [];
        ...
        function getCurrentDepth() {
            if (draggedItemIndex.value === null || !nestable.value) return 0;
        ...
        function onSortMove(e: MouseEvent) {
            ...
            function getMaxCurrentDepthWithChildren() {
                if (!draggedChildrenCount.value) return currentDepth;
                return Math.max(
            ...
            function getMinDepth(): number {
                if (draggedItemIndex.value === 0) return 0;
        ...
        function onDragOver(id: ItemID) {
            if (!draggedItemId.value || draggedItemId.value === id) return;
            ...
            function moveItem(draggingIndex: number, dragOverIndex: number) {
                let itemsToMove = items.value.splice(
                    draggingIndex,
