...
export class Whiteboard {
  ...
  public set color(color: string) {
    ...
    this.canvas.getActiveObjects().forEach((o:any) => {
      if (['path', 'arrow', 'line'].indexOf(o.type) !== -1) {
        o.set('stroke', color);
      } else {
        if (['rect', 'ellipse'].indexOf(o.type) !== -1) {
  ...
  public mount(canvasElement:HTMLCanvasElement) {
    var borderColor = getStyleVar('--theme--form--field--input--border-color-focus');
		var backgroundColor = getStyleVar('--theme--form--field--input--background');
  ...
  private updateSelection() {
    ...
    if (objects.length == 1) {
      ...
      if (objects[0].type == "image") {
        // Do nothing
      } else if (['path','line','arrow'].indexOf(objects[0].type) !== -1) {
        this._color = objects[0].stroke;
      } else if (objects[0].fill) {
        this._color = objects[0].fill;
      }
      if (['rect','ellipse'].indexOf(objects[0].type) !== -1) {
  ...
  private createEraser() {
    var brush = new fabric.EraserBrush( this.canvas );
    brush.width = 10;
    return brush;
  };
  ...
  public deleteSelected() {
    var objects = this.canvas.getActiveObjects();
    var isEditingText = objects.find((o:any) => o.type == "textbox" && o.isEditing);
    if (!isEditingText) {
      objects.forEach((o:any) => {
  ...
  private setSelectable(bool:boolean) {
    ...
    this.canvas.forEachObject(function(o:any) {
      o.selectable = bool;
    });
  ...
  private mouseUp() {
    if (!this.shapeTool || !this.shape) return;
    this.start = null;
    this.setTool("none");
    this.canvas.setActiveObject(this.shape);
    this.shape = null;
