<template>
    <div class="wrapper">
        <div class="nav">
            ...
            <v-button class="full-height-btn" v-tooltip.left="$t('fit_to_screen')" icon rounded secondary
                @click="fullHeight">
                <v-icon name="height" small />
            </v-button>

            <v-button class="full-width-btn" v-tooltip.left="$t('full_width')" icon rounded secondary
                @click="fullWidth">
        ...
        <div ref="view" class="view" :class="{ 'full-height': isFullHeight }">
            <v-notice v-if="invalidPDF" type="danger">{{ errorMessage }}</v-notice>

            <VuePdfEmbed :source="url" @loading-failed="invalidPDF = true" @rendering-failed="invalidPDF = true"
                @rendered="onRendered" :height="height" :width="width" />
        </div>
