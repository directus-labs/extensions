...
export function isComponent(component: unknown): component is ConcreteComponent {
 ...
 * @param value
 */
function nodesAsObject(value: VNodeChild | VNodeArrayChildren): value is VNodeArrayChildren | VNode {
...
function findAllVNodes(vnode: VNode, selector: string): VNode[] {
...
export function find(root: VNode, selector: string): ComponentInternalInstance[] {
	let matchingVNodes = findAllVNodes(root, selector);

	// When searching by CSS selector we want only one (topmost) vnode for each el`
	matchingVNodes = matchingVNodes.filter((vnode: VNode) => vnode.component!.parent?.vnode.el !== vnode.el);
