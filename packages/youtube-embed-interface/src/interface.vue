<template>
	...
	<v-drawer @cancel="showDrawer = false" title="Embed YouTube video" v-model="showDrawer">
		...
		<div class="layout">
			...
			<v-input autofocus full-width v-model="searchTerm" placeholder="Search YouTube videos...">
				<template #prepend>
					<v-icon
						name="search"
						class="icon-search"
			...
			>
				...
				<template #item.name="{ item }">
					<div v-html="item.snippet.title"></div>
				</template>
				...
				<template #footer>
					<div v-if="nextPageToken" class="footer">
						<div class="pagination">
							<v-button secondary :loading="loading" :disabled="loading" @click="loadMore">Load more</v-button>
						</div>
					</div>
				</template>
			</v-table>
	...
	<v-button @click="showDrawer = true">Select</v-button>
</template>
...
<script lang="ts" setup>
	...
	const preview = computed(() => {
		if (props.value) {
			var match = props.value.match(/iframe src=\"(.*?)\" height=\"(\d+)\" width=\"(\d+)\"/i);
			if (match?.length > 1) {
				return {
					src: match[1],
					height: match[2],
	...
	const videoSearch = debounce(async (value:string) => {
		...
		if (!value && !props.channelId) {
			...
			try {
				var fetchedItems = await YT.getVideos(value)
	...
	const loadMore = async () => {
		loading.value = true;
		var fetchedItems = await YT.getVideos(searchTerm.value, nextPageToken.value)
