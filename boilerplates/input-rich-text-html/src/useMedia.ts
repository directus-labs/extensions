...
export default function useMedia(
    ...
    const mediaButton = {
        ...
        tooltip: useI18n().t("wysiwyg_options.media"),
        // CORE-CHANGE end
        ...
        onSetup: (buttonApi: any) => {
            const onVideoNodeSelect = (eventApi: any) => {
                buttonApi.setActive(
                    eventApi.element.tagName === "SPAN" &&
                        eventApi.element.classList.contains(
                            "mce-preview-object"
                        )
                );
            };
    ...
    const mediaSource = computed({
        ...
        set(newSource: any) {
            ...
            mediaSelection.value.previewUrl = replaceUrlAccessToken(
                newSource,
                imageToken.value
            );
        },
    ...
    watch(embed, (newEmbed) => {
        if (newEmbed === "") {
            ...
            const tag = /<(video|audio|iframe)/g.exec(newEmbed)?.[1] as
                | "video"
                | "audio"
                | "iframe"
                | undefined;
            ...
            const width =
                Number(/width="(.*?)"/g.exec(newEmbed)?.[1]) || undefined;
            const height =
                Number(/height="(.*?)"/g.exec(newEmbed)?.[1]) || undefined;
            const type = /type="(.*?)"/g.exec(newEmbed)?.[1] || undefined;

            if (sourceUrl === undefined) return;

            // Add temporarily access token for preview
            const previewUrl = replaceUrlAccessToken(
                sourceUrl,
                imageToken.value
            );
    ...
    function saveMedia() {
        ...
        if (startEmbed.value !== "") {
            const updatedContent = editor.value
                .getContent()
                .replace(startEmbed.value, embed.value);
